program calibration
  ...
  ! i = get_sting_numbers("one11nine00zerasixasdfseven")
...
contains
  ...
  function get_numbers(string) result(res)
    ...
    integer(int32) :: res
    character(len=1) :: c
    integer(int32) :: i, first, last, n, itemp
    n = len(string)
    ...
    do i = 1, n
       ...
       if ((ichar(c) <= 57) .and. (ichar(c) >= 48)) then
          ...
          itemp = get_sting_numbers(string, i)
          if (itemp /= -1) then
             if (first == -1) then
                first = itemp
             else
  ...
  function get_sting_numbers(string, i) result(res)
    character(len=*), intent(in) :: string
    integer(int32), intent(in) :: i
    integer(int32) :: res
    character(len=5), allocatable :: numbers(:)
    integer(int32) :: j, n
    ...
    res = -1
    do j = 1, size(numbers)
       n = len(trim(numbers(j)))
       if (i + n -1 > len(string) ) cycle
       ! print *, string(i:i+n-1)
       ! print *, numbers(j)
       if (string(i:i+n-1) == trim(numbers(j))) then
          ! print *, "found number: ", j - 1
          res = j - 1
       end if
    end do
  end function get_sting_numbers
