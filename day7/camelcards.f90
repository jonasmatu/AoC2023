program camelcards
  ...
  res(1) = 0
  do i = 1, nlines
     ! print *, sortedhand(i), get_type(sortedhand(i))
...
contains
  ...
  function get_type(hand) result(type)
    ...
    elseif (maxval(kinds) == 4) then
       type = 5 ! AAAA1
       if (count_char(hand, "J") == 1) type = 6 ! AAAAA
       if (count_char(hand, "J") == 4) type = 6
    elseif (any(kinds == 3) .and. any(kinds == 2)) then
       type = 4 ! AAA11
       if (count_char(hand, "J") > 0) type = 6  ! AAAAA
    elseif (maxval(kinds) == 3) then
       type = 3 ! AAA12
       if (count_char(hand, "J") == 1) type = 5 ! AAAA1
       if (count_char(hand, "J") == 3) type = 5 ! AAAA1
    elseif (maxval(kinds) == 1) then
       type = 0 ! 12345
       if (count_char(hand, "J") == 1) type = 1 ! AA123
    elseif (count(kinds == 2) == 2) then
       type = 2 ! AAQQ1
       if (count_char(hand, "J") == 1) type = 4 ! AAA11
       if (count_char(hand, "J") == 2) type = 5 ! AAAA1
    else
       type = 1 ! AA123
       if (count_char(hand, "J") == 1) type = 3 ! AAA12
       if (count_char(hand, "J") == 2) type = 3 ! AAA12
    end if
