...
contains
  ...
  function replace(string, cin, cout) result(str)
    character(len=*), intent(in) :: string, cin, cout
    integer :: i
    ...
    do i = 1, len(string)
       if (string(i:i) /= cin) str = str // string(i:i)
       if (string(i:i) == cin) str = str // cout
    end do
  ...
  if (present(order)) then; ordr = adjustl(order); else; ordr = 'sequential'; end if ! decide on value for optional ORDER parameter
  if (present(nulls)) then; nlls = adjustl(nulls); else; nlls = 'ignore'    ; end if ! optional parameter
